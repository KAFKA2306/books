...
- NDL Searchが `NDC9` または `NDC8` を返した場合は取得元の版として保持し、NDC10へ推測変換しない
- 版情報を確認できない `NDC` も版不明のsource-reported値として保持する
...
- 表示カテゴリはNDC証跡から毎回再導出し、台帳に保存された過去のカテゴリ文字列を正準値として扱わない
...
UIではNDC10の1桁を大分類ファセットとして使い、詳細画面/APIでは `338.18` のようなフルコードを保持します。NDC9/NDC8/版不明NDCにはNDC10の大分類ラベルを推測付与しません。
...
NDL Search OpenSearch
  -> category-enrichments.json（取得証跡の唯一の正準台帳）
      -> 独自表示カテゴリ（NDCコードから毎回決定論的に派生）
      -> classifications（標準NDC API、取得元の版を保持して派生）
```
