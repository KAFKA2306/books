const COMMERCIAL_ANNOTATION = /(?:【[^】]*(?:期間限定|無料版?|モノクロ版|電子限定|先行試し読み|試し読み|見本|単話売|短編|旧版|同人版)[^】]*】|期間限定(?:無料)?|無料版|モノクロ版)/u;
const VOLUME_METADATA = /(?:\(|（)\s*[0-9０-９]{1,3}\s*(?:\)|）)|(?:^|\s)第?[0-9０-９]{1,3}\s*(?:巻|冊)(?:\s|$)|\s[0-9０-９]{1,3}\s+(?=\([^)]*(?:コミックス|コミック|DIGITAL)[^)]*\)\s*$)/iu;
const SEMANTIC_SERIAL_UNIT = /(?:^|\s)プチデザ\s*(?:\(|（)\s*[0-9０-９]{1,3}\s*(?:\)|）)/u;
const FORMAT_MARKER = /(?:Kindle版|電子書籍版|コミック版)/iu;
