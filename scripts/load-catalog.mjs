...
export async function loadCatalog(root = process.cwd()) {
  ...
  const base = { schema_version:compact.v, generated_at:compact.g, key_policy:{work:'deterministic normalized title hash',edition:'verified ISBN-13 preferred'}, stats:{...compact.s,edition_count:editions.length,holding_count:holdings.length}, works,editions,holdings };
  const issueManifest = JSON.parse(await fs.readFile(path.join(root, 'data/issue-1-books.parts.json'), 'utf8'));
