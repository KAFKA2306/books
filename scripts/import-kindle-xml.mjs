...
const parts = [];
for (let offset = 0, partNumber = 1; offset < parsed.records.length; offset += options.partSize, partNumber += 1) {
  const rows = parsed.records.slice(offset, offset + options.partSize);
...
const manifest = {
  ...
  unique_asin_count: new Set(parsed.records.map((record) => record.asin)).size,
  origin_counts: kindleOriginCounts(parsed.records),
  parts,
};
